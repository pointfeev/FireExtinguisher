...
public class WorkGiver_ExtinguishFire : WorkGiver_Scanner
{
    ...
    public override bool HasJobOnThing(Pawn pawn, Thing t, bool forced = false)
    {
        ...
        return InventoryUtils.CanEquipFireExtinguisher(pawn)
            && ((pawn.Position - fire.Position).LengthHorizontalSquared <= 225 || pawn.CanReserve(fire, 1, -1, null, forced)) && !FireIsBeingHandled(fire, pawn)
            && CastUtils.CanGotoCastPosition(pawn, fire, out _, true) && InventoryUtils.EquipFireExtinguisher(pawn);
    }
